...
(function () {
  ...
  window.addEventListener('scroll', () => {
    ...
    nav.style.backgroundColor = scrolled ? '#ffffff' : '';
...
(function () {
  function initPhilosophyScroll() {
    ...
    imgs.forEach(img => {
      ...
      img.style.height    = `${(isPhilMobile ? 57 : 60) + Math.random() * 5}px`;
    ...
    function updateVisibility() {
      if (window.innerWidth < 800) {
        items.forEach(el => {
          ...
          el.querySelectorAll('.card, .blog-card-wrap').forEach(c => c.classList.remove('is-past'));
        });
        return;
      }
      items.forEach((el, i) => {
