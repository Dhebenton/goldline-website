...
const artD = document.querySelector('.art-d');
...
function close() {
  ...
  requestAnimationFrame(() => {
    if (!isHovered) document.body.classList.remove('open');
  });
}

tgl.addEventListener('mouseenter', open);
tgl.addEventListener('mouseleave', close);
artD.addEventListener('mouseenter', open);
artD.addEventListener('mouseleave', close);
...
(function () {
  function initPhilosophyScroll() {
    const philSection = document.querySelector('#s2sc');
    ...
    imgs.forEach(img => {
      ...
      img._startY    = isLarge ? 500 + Math.random() * 40 : 270 + Math.random() * 40;
      ...
      img.style.height    = `${(isPhilMobile ? 63 : 55) + Math.random() * 5}px`;
...
(function () {
  function initAboutScroll() {
    ...
    const paras = document.querySelectorAll('#s2ab .autw p');
    if (!paras.length) return;

    const originalHTML = new Map();
    paras.forEach(p => originalHTML.set(p, p.innerHTML));
          ...
          if (isMobile) {
            ...
            while (i < chars.length) {
              if (chars[i] === ' ') {
                ...
                const pair = chars[i + 1] && chars[i + 1] !== ' '
                  ? chars[i] + chars[i + 1]
                  : chars[i];
                const span = document.createElement('span');
                span.className   = 'char reveal-text-dark';
    ...
    const aboutSection = document.querySelector('#s2ab');
    ...
    function cleanup() {
      ...
      paras.forEach(p => {
        p.innerHTML = originalHTML.get(p);
      });

      if (img) img.classList.add('revealed');

      aboutSection.classList.add('cmpt');
    ...
    function update() {
      ...
      const scrolledPast  = window.scrollY > sectionBottom;

      if (scrolledPast) {
        allChars.forEach((el, i) => { el.classList.add('revealed'); revealed[i] = 1; });
        if (img) img.classList.add('revealed');
...
(function () {
  ...
  window.addEventListener('load', () => {
    ...
    const blogSection   = document.getElementById('s2art');
...
(function () {
  function initFooter() {
    ...
    const services = document.getElementById('s2ser');
    if (!footer) return;
...
const dots = document.querySelectorAll('#menu-icon .d');
const frames = [
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1 ],
  ['#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1 ],
  ['#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1 ],
  ['#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',1 ],
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',.5, '#1F1F1F',1 ],
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',1,  '#1F1F1F',.5, '#1F1F1F',.5],
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',.5],
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  'white',   .35],
  ['#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  '#1F1F1F',1,  'white',  .28, '#1F1F1F',1,  'white',  .28, 'white',   1  ],
  ['#1F1F1F',1,  '#1F1F1F',1,  'white',  .28, '#1F1F1F',1,  'white',  .28, 'white', .28, 'white',  .28, 'white',  .28, 'white',   1  ],
  ['#1F1F1F',1,  'white',  .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',   1  ],
  ['white',  .25, 'white', .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',   1  ],
  ['white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',  1,  'white',  .28, 'white',   1  ],
];

function applyFrame(f) {
  dots.forEach((dot, i) => {
    dot.setAttribute('fill', f[i * 2]);
    dot.setAttribute('fill-opacity', f[i * 2 + 1]);
  });
}
...
function animate(toOpen) {
  ...
  function step(ts) {
    ...
    progress = startProgress + delta * eased;
...
menuBtn.addEventListener('click', () => {
  const opening = !document.body.classList.contains('mob-open');

  if (opening) {
    document.body.classList.remove('mob-closing');
    document.body.classList.add('mob-open');
    animate(true);
  } else {
    document.body.classList.add('mob-closing');
    animate(false);

    const md = document.querySelector('nav .m-d');
    md.addEventListener('transitionend', () => {
